...
def _check(assertion: dict, report: dict) -> tuple[bool, str]:
    ...
    if check == "memo_no_actual_transaction_claim":
        memo = (report.get("memo_markdown") or "").lower()
        # The memo MUST include the proxy disclaimer, not claim real transaction data.
        # Flag if it uses phrasing that implies real transaction prices without qualification.
...
def run_case(session: "requests.Session", base: str, case: dict) -> dict:
    ...
    if r.status_code not in (200, 201):
        return {"id": case_id_label, "status": "ERROR", "reason": f"Case creation failed: {r.status_code} {r.text[:200]}"}
    case_data = r.json()
    ...
    if not case_data:
        return {"id": case_id_label, "status": "ERROR", "reason": "Ingestion timed out or failed"}

    # 3. Run workflow
    r = session.post(_api(base, f"/api/v1/workflow/{case_id}/run"), timeout=15)
    if r.status_code not in (200, 201):
        return {"id": case_id_label, "status": "ERROR", "reason": f"Workflow start failed: {r.status_code}"}

    # 4. Wait for human-review checkpoint
    case_data = _wait_for_status(session, base, case_id, ["awaiting_human_review", "completed", "rejected"])
    if not case_data:
        return {"id": case_id_label, "status": "ERROR", "reason": "Workflow timed out waiting for human-review"}
    ...
    if case_data.get("workflow_status") == "awaiting_human_review":
        ...
        if r.status_code not in (200, 201):
            return {"id": case_id_label, "status": "ERROR", "reason": f"Resume failed: {r.status_code}"}

    # 6. Wait for final report
    case_data = _wait_for_status(session, base, case_id, ["completed", "rejected"])
    if not case_data:
        return {"id": case_id_label, "status": "ERROR", "reason": "Report generation timed out"}
    ...
    # 8. Optional LLM evals (Ragas / DeepEval) — skipped silently if unavailable
    llm_eval_result = _run_llm_eval(report)
...
def _run_llm_eval(report: dict) -> dict:
    ...
    try:
        import sys
        import os
        # Attempt to import from the backend eval module if on the path
        sys.path.insert(0, str(Path(__file__).parent.parent / "backend"))
        from app.eval.deepeval_runner import run_deepeval_smoke, check_citations_present
        ...
        ragas_result = run_ragas_eval(ragas_cases)
...
def main() -> int:
    ...
    parser.add_argument("--api", default=DEFAULT_API, help="API base URL (default: http://localhost:8000)")
    ...
    for case in cases:
        ...
        try:
            ...
            result = {"id": case["id"], "status": "ERROR", "reason": str(exc)}
        ...
        if result.get("estimated_value"):
            print(f"  → value=${result['estimated_value']:,.0f}  "
                  f"conf={result.get('confidence_score', 0):.2f}  "
                  f"risk={result.get('risk_score', 0):.0f}  "
    ...
    print("\n" + "=" * 70)
    print(f"RESULTS: {passed}/{total} passed  |  {failed} failed  |  {errors} error(s)")
    if passed == total:
